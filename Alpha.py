...
def max_value(node,alpha, beta):
    ...
    for roots in children:
        minimum = min_value(roots,alpha, beta)
...
def min_value(node,alpha, beta):
    ...
    children = [node.getRightChild(), node.getLeftChild()]
    for roots in children:
        maximum = max_value(roots,alpha, beta)
...
def alphabeta(BTree):
    ...
    children = [BTree.getRightChild(), BTree.getLeftChild()]
    winner = None
    for prune in children:
        output = min_value(prune, alpha, beta)
        if output > alpha:
            winner = output
            best = prune
